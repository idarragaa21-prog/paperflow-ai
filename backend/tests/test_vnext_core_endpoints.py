...
class TestVNextCoreFlow:
    async def test_extraction_matrix_to_meta_run_pipeline(
        ...
        class DummyAsyncClientResponse:
        ...
        class DummyAsyncClientMeta:
            ...
            async def post(self, url, **kwargs):
                if "run-analysis" in url:
                    return DummyAsyncClientResponse()
                from httpx import AsyncClient as RealClient

                async with RealClient() as client:
                    return await client.post(url, **kwargs)

        import httpx

        monkeypatch.setattr(httpx, "AsyncClient", DummyAsyncClientMeta)
        ...
        assert any(
            item.startswith("forest_") or item.startswith("rob_")
