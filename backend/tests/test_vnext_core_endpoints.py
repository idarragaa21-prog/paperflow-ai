    ...
    async def test_extraction_matrix_to_meta_run_pipeline(
        ...
        class MockREngineClient:
            ...
            async def post(self, url, json=None, timeout=None, **kwargs):
                ...
                # Fallback to normal behavior for other mocked services if needed
                class Dummy:
                    status_code = 500
        ...
        monkeypatch.setattr(httpx, "AsyncClient", MockREngineClient)
        ...
        assert any(
            item in artifact_types for item in {"session_info", "session_info_txt"}
        )
        assert any("summary" in item for item in artifact_types)
        assert any(
            item.startswith("figure_")
            or item.startswith("forest_")
            or item.startswith("rob_")
