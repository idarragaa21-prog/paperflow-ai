        ...
        class MockAsyncClient(AsyncClient):
            ...
            async def post(self, url, **kwargs):
                if "run-analysis" in str(url):
                    class DummyResponseRun:
                        status_code = 200
                        def json(self):
                            return {"summary": {"rows_total": 2}, "figure_artifacts": {"forest": {"svg": "PHN2Zz48L3N2Zz4="}}}
                        def raise_for_status(self): pass
                    return DummyResponseRun()
                class DummyResponseFallback:
                    status_code = 200
                    def json(self): return {}
                    def raise_for_status(self): pass
                return DummyResponseFallback()
        monkeypatch.setattr("app.services.meta_runs_service.httpx.AsyncClient", MockAsyncClient)
