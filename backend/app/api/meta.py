...
async def create_batch(
    ...
    # Pre-calculate hashes to avoid N+1 query and memory issues
    file_hashes = []
    ...
    # Pre-fetch existing papers to avoid N+1 queries
    existing_papers_dict = {}
    if file_hashes:
        existing_q = await db.execute(
            select(Paper)
            .where(Paper.project_id == params.project_id)
            .where(Paper.content_hash.in_(file_hashes))
        )
    ...
    for i, f in enumerate(params.files):
        ...
        if not paper:
            ...
            # Update dictionary with new paper to handle intra-batch duplicates
