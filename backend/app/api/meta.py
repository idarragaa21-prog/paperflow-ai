...
async def create_batch(
    ...
    # 2. Fetch existing papers in a single query (prevent N+1)
    existing_items = {}
    if content_hashes:
        ...
        existing_items = {p.content_hash: p for p in existing_q.scalars().all()}

    # 3. Create missing papers and queue items
    for f, content_hash in zip(params.files, content_hashes):
        paper = existing_items.get(content_hash)
        if not paper:
            content = await f.read()
            ...
            # Add to dictionary for intra-batch duplicate handling
            existing_items[content_hash] = paper
