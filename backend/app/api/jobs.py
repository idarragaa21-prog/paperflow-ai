...
async def get_job(
    ...
    try:
        ...
        logger.exception("Unexpected error while querying job status")
        raise HTTPException(status_code=500, detail="Error consultando job")
...
async def retry_job(
    ...
    try:
        ...
        logger.exception("Unexpected error while retrying job")
        raise HTTPException(status_code=500, detail="No se pudo reintentar el job") from exc
