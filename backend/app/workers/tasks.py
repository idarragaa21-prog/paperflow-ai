        ...
        try:
            ...
            async with async_session_maker() as db:
                        ...
                        "meta_runs": [
                            {
                                ...
                                "status": item.status,
                                "created_at": item.created_at.isoformat() if item.created_at else None,
                                "artifact_count": len(item.artifacts or []),
                            }
                        ...
                        "writing_documents": [
                            {
                                ...
                                "mode": item.mode,
                                "status": item.status,
                                "section_count": len(item.sections or []),
                                "created_at": item.created_at.isoformat() if item.created_at else None,
                            }
                            for item in writing_documents
                        ],
                        "clinical_consults": [
                            {
                                "id": str(item.id),
                ...
                with zipfile.ZipFile(str(out_abs), "w", compression=zipfile.ZIP_DEFLATED) as z:
                    ...
                    # Writing documents as markdown
                    for doc in writing_documents:
                        safe_title = _safe_name(doc.title)[:80]
                        prefix = f"{root}/writing/{doc.id}_{safe_title}"
                        for section in doc.sections or []:
                            safe_section = _safe_name(section.section_key or section.heading)[:64]
                            z.writestr(f"{prefix}/{safe_section}.md", section.content_markdown or "")

                    # Clinical consult outputs
