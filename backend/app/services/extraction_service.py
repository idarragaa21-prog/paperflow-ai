...
_COUNTRY_THE_RE = re.compile(r"^(?:the)\s+", flags=re.IGNORECASE)
_COUNTRY_TRAILING_RE = re.compile(
    rf"\s+(?:{'|'.join(COUNTRY_TRAILING_TOKENS)})\b.*$", flags=re.IGNORECASE
)
...
_SAMPLE_RE = re.compile(r"\b(?:n|sample size)\s*[=:]\s*(\d+)")
_INTERVENTION_RE = re.compile(r"\bintervention[s]?:\s*([^\n.]+)", flags=re.IGNORECASE)
_COMPARATOR_RE = re.compile(r"\bcomparator[s]?:\s*([^\n.]+)", flags=re.IGNORECASE)
_OUTCOME_RE = re.compile(r"\boutcome[s]?:\s*([^\n.]+)", flags=re.IGNORECASE)
_RESULTS_RE = re.compile(r"\bresults?\b[:\s]*([^\n]{20,250})", flags=re.IGNORECASE)
_FOLLOW_UP_RE = re.compile(r"\bfollow[- ]?up\b[:\s]*([^\n.]+)", flags=re.IGNORECASE)
_LIMITATIONS_RE = re.compile(
    r"\blimitations?\b[:\s]*([^\n]{15,250})", flags=re.IGNORECASE
)
_CONCLUSIONS_RE = re.compile(
    r"\bconclusions?\b[:\s]*([^\n]{15,250})", flags=re.IGNORECASE
)

_YEAR_VALIDATOR_RE = re.compile(r"(19|20)\d{2}")
_SAMPLE_VALIDATOR_RE = re.compile(r"\d+")
...
def _normalize_country_name(raw_value: str) -> str | None:
    ...
    cleaned = _COUNTRY_THE_RE.sub("", cleaned)
    cleaned = _COUNTRY_TRAILING_RE.sub("", cleaned).strip(" ,;:.")
    if not cleaned:
        return None
...
def _extract_value(name: str, paper: Paper, text: str) -> str | None:
    ...
    if name == "design":
        ...
        return None
    if name == "sample":
        match = _SAMPLE_RE.search(lowered)
    ...
    if name == "follow_up":
        ...
        return match.group(1).strip() if match else None
    if name == "limitations":
        match = _LIMITATIONS_RE.search(text)
        return match.group(1).strip() if match else None
    if name == "conclusion":
        match = _CONCLUSIONS_RE.search(text)
        return match.group(1).strip() if match else None
    if name == "population":
...
def _validate_field(name: str, value: str | None) -> list[str]:
    ...
    if not value:
        return issues
    if name == "year":
        if not _YEAR_VALIDATOR_RE.fullmatch(value.strip()):
            issues.append("invalid_year")
    if name == "sample":
        if not _SAMPLE_VALIDATOR_RE.fullmatch(value.strip()):
