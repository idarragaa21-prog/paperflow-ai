...
def _relevance_score(
    ...
    score = 0.0
    # ⚡ Bolt: Query tokens are pre-computed in callers and passed here to avoid redundant regex O(N) overhead during ranking loops.
    if query_tokens is None:
        query_tokens = set(re.sub(r"[^a-z0-9 ]+", "", query.lower()).split())
    title_tokens = set(
        re.sub(r"[^a-z0-9 ]+", "", (item.get("title") or "").lower()).split()
    )
...
async def federated_search(
    ...
    # ⚡ Bolt: Pre-compute tokenized query once to avoid O(N) regex compilations inside the sorted() key.
    query_tokens = set(re.sub(r"[^a-z0-9 ]+", "", query.lower()).split())
    ranked = sorted(
        merged,
        key=lambda item: (
            _relevance_score(item, query, query_tokens),
