...
def _relevance_score(
    ...
    if query_tokens is None:
        query_tokens = set(_NON_ALPHANUMERIC_RE.sub("", query.lower()).split())
    title_tokens = set(
        _NON_ALPHANUMERIC_RE.sub("", (item.get("title") or "").lower()).split()
    )
...
async def federated_search(
    ...
    # Pre-tokenize query to avoid redundant regex substitution in sorting
    query_tokens = set(_NON_ALPHANUMERIC_RE.sub("", query.lower()).split())
