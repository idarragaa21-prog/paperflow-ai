...
def _relevance_score(
    ...
    if query_tokens is None:
        query_tokens = set(_RE_ALPHA_NUM.sub("", query.lower()).split())
    title_tokens = set(_RE_ALPHA_NUM.sub("", (item.get("title") or "").lower()).split())
...
async def federated_search(
    ...
    ranked = sorted(
        merged,
        key=lambda item: (
            _relevance_score(item, query, query_tokens=query_tokens),
    ...
    for item in ranked[:max_results]:
        ...
        enriched["relevance_score"] = _relevance_score(
            enriched, query, query_tokens=query_tokens
        )
