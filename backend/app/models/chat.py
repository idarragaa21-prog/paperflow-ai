...
class ChatSession(Base, TimestampMixin):
    ...
    paper_id: Mapped[uuid.UUID | None] = mapped_column(Uuid(as_uuid=True, native_uuid=True), ForeignKey("papers.id", ondelete="CASCADE"), nullable=True)
...
class ChatMessage(Base, TimestampMixin):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True, native_uuid=True), primary_key=True, default=uuid.uuid4)
    session_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True, native_uuid=True), ForeignKey("chat_sessions.id", ondelete="CASCADE"), nullable=False)
    user_id: Mapped[uuid.UUID | None] = mapped_column(Uuid(as_uuid=True, native_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
...
class RetrievedChunk(Base, TimestampMixin):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True, native_uuid=True), primary_key=True, default=uuid.uuid4)
    message_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True, native_uuid=True), ForeignKey("chat_messages.id", ondelete="CASCADE"), nullable=False)
    paper_chunk_id: Mapped[uuid.UUID | None] = mapped_column(Uuid(as_uuid=True, native_uuid=True), ForeignKey("paper_chunks.id", ondelete="SET NULL"), nullable=True)
