...
from sqlalchemy import ForeignKey, Integer, JSON, String, Text, Table, Column, Uuid, Index
...
presentation_papers = Table(
    "presentation_papers",
    Base.metadata,
    Column("presentation_id", Uuid(as_uuid=True, native_uuid=True), ForeignKey("presentations.id", ondelete="CASCADE"), primary_key=True),
    Column("paper_id", Uuid(as_uuid=True, native_uuid=True), ForeignKey("papers.id", ondelete="CASCADE"), primary_key=True),
)
...
class Presentation(Base, TimestampMixin):
    ...
    references_used: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)
