...
class ClinicalSheet(Base):
    ...
    project_id: Mapped[uuid.UUID | None] = mapped_column(Uuid(as_uuid=True, native_uuid=True), ForeignKey("projects.id", ondelete="SET NULL"), nullable=True)
    ...
    references_json: Mapped[list[dict] | dict | None] = mapped_column(JSON, nullable=True)
