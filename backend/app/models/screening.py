...
class ScreeningDecision(Base, TimestampMixin):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True, native_uuid=True), primary_key=True, default=uuid.uuid4)
    batch_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True, native_uuid=True), ForeignKey("screening_batches.id", ondelete="CASCADE"), nullable=False)
    ...
    reason_id: Mapped[uuid.UUID | None] = mapped_column(Uuid(as_uuid=True, native_uuid=True), ForeignKey("eligibility_reasons.id", ondelete="SET NULL"), nullable=True)
...
class ReviewFlow(Base, TimestampMixin):
    ...
    prisma_counts_json: Mapped[dict | None] = mapped_column(JSON, nullable=True)
