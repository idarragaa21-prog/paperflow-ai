...
class DraftSection(Base, TimestampMixin):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True, native_uuid=True), primary_key=True, default=uuid.uuid4)
    draft_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True, native_uuid=True), ForeignKey("drafts.id", ondelete="CASCADE"), nullable=False)
...
class DraftCitation(Base, TimestampMixin):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True, native_uuid=True), primary_key=True, default=uuid.uuid4)
    draft_section_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True, native_uuid=True), ForeignKey("draft_sections.id", ondelete="CASCADE"), nullable=False)
    reference_item_id: Mapped[uuid.UUID | None] = mapped_column(Uuid(as_uuid=True, native_uuid=True), ForeignKey("reference_items.id", ondelete="SET NULL"), nullable=True)
    paper_citation_span_id: Mapped[uuid.UUID | None] = mapped_column(Uuid(as_uuid=True, native_uuid=True), ForeignKey("paper_citation_spans.id", ondelete="SET NULL"), nullable=True)
