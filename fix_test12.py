...
# The issue is simple: the test tries to reach `settings.R_ENGINE_URL` which fails because it's not running in CI.
# We can mock `httpx.AsyncClient.post` in `app.services.meta_runs_service` but we must apply the decorator correctly!
