...
export default function ClinicalPage() {
  ...
  // ── Live paper count for selected project ─────────────────────────────────
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 980 }}>
      ...
      {selectedProject && (
        <div className="rc-card" style={{ background: 'rgba(16,185,129,0.06)', border: '1px solid rgba(16,185,129,0.2)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ fontSize: 13 }}>
            🔗 <b>{selectedProject.title}</b>
            {projectPaperCount !== undefined && (
              <span style={{ marginLeft: 10, fontSize: 12, fontWeight: 700, padding: '2px 8px', borderRadius: 6,
                background: projectPaperCount > 0 ? 'rgba(16,185,129,0.12)' : 'rgba(0,0,0,0.06)',
                color: projectPaperCount > 0 ? '#059669' : 'var(--rc-muted)' }}>
                {projectPaperCount > 0
                  ? `✓ ${projectPaperCount} paper${projectPaperCount !== 1 ? 's' : ''} contribuirán a la evidencia`
                  : '⚠ Sin papers — el pipeline usará solo fuentes online'}
              </span>
            )}
          </div>
      ...
      <div className="rc-card">
        ...
        {/* Project selector — always visible at the top */}
        {projects.length > 0 && (
          <div style={{ marginBottom: 12 }}>
            <div className="rc-kicker">Proyecto (opcional) — sus papers añaden evidencia local</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <select className="rc-select" value={projectId} onChange={(e) => setProjectId(e.target.value)} style={{ flex: 1 }}>
                <option value="">(ninguno — solo fuentes online)</option>
                {projects.map(p => <option key={p.id} value={p.id}>{p.title}</option>)}
              </select>
              {projectId && projectPaperCount !== undefined && (
                <span style={{ fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 8, whiteSpace: 'nowrap',
                  background: projectPaperCount > 0 ? 'rgba(16,185,129,0.1)' : 'rgba(0,0,0,0.06)',
                  color: projectPaperCount > 0 ? '#059669' : 'var(--rc-muted)',
                  border: `1px solid ${projectPaperCount > 0 ? 'rgba(16,185,129,0.25)' : 'var(--rc-border)'}` }}>
                  {projectPaperCount > 0 ? `${projectPaperCount} papers` : 'Sin papers'}
                </span>
              )}
            </div>
          </div>
        ...
        <textarea className="rc-textarea" value={topic} onChange={(e) => setTopic(e.target.value)}
          placeholder="Ej: Fractura por estrés del quinto metatarsiano — tratamiento conservador vs. quirúrgico" style={{ minHeight: 90 }} />
        ...
        {advanced && (
          <div style={{ marginTop: 10, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              ...
              <textarea className="rc-textarea" value={context} onChange={(e) => setContext(e.target.value)}
                placeholder="Edad, deporte, comorbilidades, escenario clínico…" style={{ minHeight: 70 }} />
            </div>
            <div>
        ...
        <div style={{ height: 14 }} />
        <button className="rc-btn rc-btn--primary" disabled={!canGenerate || generateMut.isPending || Boolean(jobId)}
          onClick={() => generateMut.mutate()} style={{ padding: '9px 20px', fontSize: 14 }}>
      ...
      <div className="rc-card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          ...
          <button className="rc-btn" style={{ fontSize: 12, padding: '4px 10px' }}
            onClick={() => qc.invalidateQueries({ queryKey: sheetsKey })} disabled={sheetsLoading}>
            {sheetsLoading ? '…' : '↻'}
          </button>
        </div>
        {sheets.length === 0 && !sheetsLoading
          ...
          : sheets.map(s => (
            <button key={s.id} onClick={() => navigate(`/clinical/sheets/${s.id}`)} className="rc-btn"
              style={{ textAlign: 'left', padding: 12, width: '100%', marginBottom: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                <div style={{ fontWeight: 850 }}>{s.topic}</div>
                {s.project_id && (
                  <span className="rc-badge" style={{ fontSize: 10, flexShrink: 0 }}>
                    🔗 {projects.find(p => p.id === s.project_id)?.title || 'Project'}
                  </span>
                )}
              </div>
