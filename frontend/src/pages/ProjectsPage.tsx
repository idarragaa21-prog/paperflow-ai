...
function EmptyState({ archived }: { archived: boolean }) {
  return (
    <div style={{ textAlign:'center',padding:'80px 24px' }}>
      <svg width="96" height="96" viewBox="0 0 96 96" fill="none" style={{ margin:'0 auto 20px',display:'block' }}>
        ...
        {!archived && <>
          <circle cx="76" cy="76" r="18" fill="var(--rc-surface)" stroke="rgba(99,102,241,0.3)" strokeWidth="1.5"/>
      ...
      <div style={{ fontWeight:700,fontSize:16,fontFamily:'var(--font-display)',letterSpacing:'-0.02em',color:'var(--rc-text)' }}>
...
export default function ProjectsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      setProjects(list);
      // Load counts for active projects
  ...
  async function create() {
    ...
    try {
      await api.post('/projects', { title: newTitle.trim(), description: newDesc.trim() || null, clinical_area: newArea.trim() || null });
      setNewTitle(''); setNewDesc(''); setNewArea(''); setShowCreate(false);
      await load();
  ...
  return (
    <div className="rc-page-enter" style={{ display:'flex',flexDirection:'column',gap:20 }}>
      ...
      <div style={{ display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:12,flexWrap:'wrap' }}>
        ...
        <button className="rc-btn rc-btn--primary" onClick={() => setShowCreate(!showCreate)} style={{ flexShrink:0 }}>
        ...
        <div className="rc-card" style={{ borderColor:'rgba(99,102,241,0.25)',background:'rgba(99,102,241,0.02)' }}>
          ...
          <div style={{ display:'flex',flexDirection:'column',gap:12 }}>
            <div>
              ...
              <input className="rc-input" value={newTitle} onChange={e=>setNewTitle(e.target.value)}
                placeholder="e.g. Distal radius fracture outcomes in older adults" autoFocus
                onKeyDown={e=>e.key==='Enter'&&create()}/>
            </div>
            <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:12 }}>
              <div>
                <div className="rc-kicker">Clinical area</div>
                <input className="rc-input" value={newArea} onChange={e=>setNewArea(e.target.value)} placeholder="e.g. Orthopedics"/>
              </div>
              <div>
                <div className="rc-kicker">Description</div>
                <input className="rc-input" value={newDesc} onChange={e=>setNewDesc(e.target.value)} placeholder="Optional"/>
              </div>
            </div>
      ...
      {error && <div className="rc-error rc-card" style={{ borderColor:'var(--rc-danger-border)',background:'var(--rc-danger-bg)' }}>{error}</div>}

      {/* Toolbar */}
      <div style={{ display:'flex',gap:10,alignItems:'center',flexWrap:'wrap' }}>
        <div style={{ flex:'1 1 220px',position:'relative' }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--rc-muted)" strokeWidth="2" strokeLinecap="round" style={{ position:'absolute',left:10,top:'50%',transform:'translateY(-50%)',pointerEvents:'none' }}>
            <circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/>
          </svg>
          <input className="rc-input" value={search} onChange={e=>setSearch(e.target.value)}
            placeholder="Search projects…" style={{ paddingLeft:32 }}/>
        </div>
        <div style={{ display:'flex',gap:2,background:'rgba(26,25,41,0.06)',padding:3,borderRadius:10 }}>
          {(['active','archived'] as const).map(t => (
            <button key={t} onClick={()=>setTab(t)} style={{
              ...
              background: tab===t ? 'var(--rc-surface)' : 'transparent',
              color: tab===t ? 'var(--rc-text)' : 'var(--rc-muted)',
              boxShadow: tab===t ? 'var(--rc-shadow-sm)' : 'none',
      ...
      {loading ? (
        ...
        <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:12 }}>
          {filtered.map(p => {
            ...
            return (
              <div key={p.id} className="rc-card rc-card--hover" style={{ display:'flex',flexDirection:'column',gap:10,position:'relative' }}>
                {/* Card header */}
                <div style={{ display:'flex',gap:8,alignItems:'flex-start',justifyContent:'space-between' }}>
                  <div style={{ flex:1,cursor:'pointer' }} onClick={()=>navigate(`/projects/${p.id}/research`)}>
                    ...
                    {p.description && <div className="rc-help" style={{ marginTop:2 }}>{p.description}</div>}
                  </div>
                  {!p.archived && (
                    <button className="rc-btn rc-btn--ghost rc-btn--sm" title="Archive project"
                      onClick={()=>{ if(confirm(`Archive "${p.title}"?`)) archive(p.id); }}
                ...
                {/* Open button */}
                {!p.archived && (
                  <button className="rc-btn rc-btn--primary rc-btn--sm" onClick={()=>navigate(`/projects/${p.id}/research`)}
                    style={{ alignSelf:'flex-start',marginTop:2 }}>
                    Open →
                  </button>
                )}
                {p.archived && (
                  <span className="rc-tag rc-tag--slate" style={{ alignSelf:'flex-start' }}>Archived</span>
                )}
              </div>
            );
          })}
