...
export default function PapersPage() {
  ...
  async function deleteFailedPapers() {
    const failed = papers.filter(p => (p.processing_status || '').toLowerCase() === 'failed');
    if (!failed.length) return;
    const ok = await confirm({ title: '¿Eliminar fallidos?', body: `Se eliminarán ${failed.length} papers que fallaron al procesarse.`, confirmText: 'Eliminar todos', danger: true });
    if (!ok) return;

    for (const p of failed) {
      try { await api.delete(`/papers/${p.id}`); } catch { /* ignore */ }
    }
    toast.success('Eliminados', `${failed.length} papers fallidos han sido eliminados.`);
    invalidate();
  }
    ...
    <div className="rc-page-enter" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div className="rc-row" style={{ justifyContent: 'space-between' }}>
        <div className="rc-row" style={{ gap: 8 }}>
          ...
          {failedCount > 0 && <button className="rc-btn" style={{ padding: '8px 12px', fontSize: 12, color: 'var(--rc-danger)', borderColor: 'rgba(239,68,68,0.3)' }} onClick={deleteFailedPapers}>Limpiar Fallidos ({failedCount})</button>}
