...
## 2024-06-14 - Prevent Username Enumeration via Timing Attacks
**Vulnerability:** The `/login` endpoint returned a fast response if a user did not exist, but took significantly longer to hash and verify a password if the user did exist.
**Learning:** This timing difference can be exploited by an attacker to enumerate valid email addresses/usernames in the system, even if the error message itself was safely generic ("Credenciales inválidas").
**Prevention:** Always call `pwd_context.dummy_verify()` when a user is not found to simulate the time delay of an actual password check, ensuring constant-time execution regardless of user existence.
