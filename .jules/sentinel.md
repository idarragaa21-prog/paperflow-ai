...
## 2024-05-22 - Prevent Username Enumeration in Login
**Vulnerability:** The login endpoint returns early when a user is not found, leading to a timing attack that allows an attacker to enumerate valid email addresses.
**Learning:** Even if a generic error message is returned, the difference in response time between verifying a password hash (expensive) and simply returning an error (fast) leaks information.
**Prevention:** Always call a constant-time dummy verification function like `pwd_context.dummy_verify()` when the user is not found before returning the error.
