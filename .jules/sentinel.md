...
## 2026-04-10 - [Information Disclosure via HTTP 500 Responses]
**Vulnerability:** The API endpoints `/jobs/{job_id}`, `/jobs/{job_id}/retry`, and `/papers/download` were directly returning raw exception details (`Exception` string representations) in their HTTP 500/404 response details using f-strings.
**Learning:** API endpoints must never return raw, unhandled internal exception details or stack traces to the client. This exposes the internal state, logic, and potentially sensitive information about the backend infrastructure.
**Prevention:** Catch generic exceptions, log the full traceback securely on the server using `logger.exception`, and return a sanitized, generic error message (e.g., `"Error consultando job"`) to the client.
